...
class Communication {
	...
	socketConnect() {
		...
		socket.ws.onclose = function (event) {
			...
			if (was_connected) {
				...
				// socket was already disconnected, so increase retry delay (slow backoff)
				self.sockReconnDelayCur = Math.min( self.sockReconnDelayCur + 1, self.sockReconnDelayMax );
			}
	...
	handleSocketMessage(json) {
		...
		switch (cmd) {
			...
			case 'joined':
				...
				socket.auth = true;
				// save base URL separately, so it survives socket disconnects during job prep
				this.base_url = (socket.secure ? 'https:' : 'http:') + '//' + socket.host + ':' + socket.port;
	...
	updateConfig(updates) {
		...
		try {
			...
			Tools.writeFileAtomicSync( this.config.configFile, JSON.stringify(temp_config, null, "\t") + "\n", { mode: 0o600, flush: true } );
		}
		catch (err) {
