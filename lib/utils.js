...
class Utils {
	...
	cleanEnv() {
		...
		if (!Tools.isWindows) {
			...
			paths.unshift( Path.join( env.HOME || '/root', '.local', 'bin' ) );
			paths.unshift( Path.join( process.cwd(), 'bin' ) );
	...
	getProcsFast(callback) {
		...
		else if (this.platform.freebsd) {
			// FreeBSD uses BSD process selection flags, not the GNU "-e" form
			// PID  PPID  %CPU %MEM PRI      VSZ    RSS NI     ELAPSED STAT TTY      USER GROUP COMMAND
			ps_args = ['-ax', '-o', 'pid,ppid,%cpu,%mem,pri,vsz,rss,nice,etime,state,tty,user,group,args'];
			ps_opts.env.LC_ALL = 'C';
		}
	...
	upgradeSatellite() {
		...
		var timeout_sec = this.config.get('upgrade_timeout_sec') || 60;
		var timer = setTimeout( function() {
			...
			try {
				...
				self.socket.send('critical', { description: `Satellite upgrade did not complete within ${timeout_sec} seconds.`, details });
	...
	parseDockerStatRow(row) {
		// convert docker stat format to raw bytes and otherwise sortable values
		// {"BlockIO":"21.3GB / 2.4GB","CPUPerc":"1.37%","Container":"260b34079fe6970132aeab5cd5d6f738d456684603e8db2b9126fff6c23c4614","ID":"260b34079fe6","MemPerc":"2.14%","MemUsage":"171.3MiB / 7.807GiB","Name":"xyops01","NetIO":"523MB / 146MB","PIDs":"23"}
		
		var cont = {
			id: row.ID,
			name: row.Name,
			cpu: parseFloat( row.CPUPerc ),
			pids: parseInt( row.PIDs ),
			disk_read: 0,
			disk_write: 0,
			disk_total: 0,
			mem_usage: 0,
		...
		if (row.BlockIO) {
			// the BlockIO numbers are in decimal units (1000)
			// "BlockIO":"21.3GB / 2.4GB"
			var parts = String(row.BlockIO).split(/\s*\/\s*/);
			cont.disk_read = Tools.getBytesFromText( String(parts[0]), 1000 );
			cont.disk_write = Tools.getBytesFromText( String(parts[1]), 1000 );
			cont.disk_total = cont.disk_read + cont.disk_write;
		}
		if (row.NetIO) {
			// the NetIO numbers are in decimal units (1000)
			// "NetIO":"523MB / 146MB"
			var parts = String(row.NetIO).split(/\s*\/\s*/);
			cont.net_read = Tools.getBytesFromText( String(parts[0]), 1000 );
			cont.net_write = Tools.getBytesFromText( String(parts[1]), 1000 );
			cont.net_total = cont.net_read + cont.net_write;
		}
		if (row.MemUsage) {
			// the MemUsage numbers are in binary units (1024)
			// "MemUsage":"171.3MiB / 7.807GiB"
			var parts = String(row.MemUsage).split(/\s*\/\s*/);
			cont.mem_usage = Tools.getBytesFromText( String(parts[0]), 1024 );
			cont.mem_total = Tools.getBytesFromText( String(parts[1]), 1024 );
		}
