...
const MAX_THREADS = os.availableParallelism ? os.availableParallelism() : 1;
...
class Monitor {
	...
	getBasicServerInfo(callback) {
		...
		// only include sniffed iface ip if not localhost-ish
		if (this.server.ip && !this.server.ip.match(/^(127\.0\.0\.\d+|169\.254\.\d+\.\d+|::1)$/)) {
			info.ip = this.server.ip;
		}
	...
	runQuickMonitors(opts = {}, callback = null) {
		...
		async.parallel([
			...
			function(callback) {
				...
				info.commands = {};
				self.logMonitor(10, "Calling custom commands...");
				perf.begin('commands');
				
				// filter commands by server groups and quick flag
				var commands = self.commands.filter( function(command) {
					return command.quick && (!command.groups.length || Tools.includesAny(command.groups, self.groups));
				} );
				
				async.eachLimit( commands, self.config.get('monitor_plugin_concurrency') || MAX_THREADS,
					function(command, callback) {
						self.logMonitor(10, "Calling custom command: " + command.id, command);
						// copy command to override timeout
						self.runMonitorCommand({ ...command, timeout: 1 }, function(result) {
							if (String(result).match(/^Error\:/)) self.logError('monitor', "Monitor Command Failed: " + result);
							info.commands[ command.id ] = result;
							self.logMonitor(10, "Custom command response: " + command.id, { result });
							callback();
						} );
					},
					function() {
						perf.end('commands');
						self.logMonitor(10, "Done with custom commands");
						callback();
					}
				); // async.eachLimit
			},
	...
	runMonitors(opts, callback) {
		...
		// never allow full monitor cycles to overlap
		if (this.config.get('monitoring_enabled')) {
			if (this.monitorInProgress) {
				// only log the first skipped cycle, to avoid flooding the error log
				if (!this.monitorOverrunLogged) {
					this.monitorOverrunLogged = true;
					this.logError('monitor', "Previous monitor cycle is still running, skipping this cycle");
				}
				return callback();
			}
			this.monitorInProgress = true;
			this.monitorOverrunLogged = false;
		}
		...
		var max_sleep_ms = opts.max_sleep_ms || (Tools.clamp(this.numServers, 1, 1000) * 29);
		var sleep_ms = 1000 + (this.hostID % (max_sleep_ms || 1));
		
		if (!this.config.get('monitoring_enabled')) {
			// monitoring disabled: send back dummy record
			info.data.dummy = true;
			info.data.os = {};
			info.data.memory = {};
			info.data.cpu = {};
			info.data.mounts = {};
			...
			this.logMonitor(9, "Sleeping for " + sleep_ms + " ms");
			perf.begin('sleep');
			
			setTimeout( function() {
				perf.end('sleep');
				perf.end();
				info.data.metrics = perf.metrics();
		...
		async.series([
			...
			function(callback) {
				// gpus
				if (!self.config.get('graphics_enabled')) return process.nextTick( callback );
				self.logMonitor(9, "Calling graphics...");
				perf.begin('graphics');
				si.graphics( function(data) {
					perf.end('graphics');
					info.data.graphics = data;
					self.logMonitor(9, "graphics response", data);
					callback();
				} );
			},
			function(callback) {
				// docker stats, if available and not explicitly disabled
				if (!self.dockerBin || (self.config.get('docker_enabled') === false)) return process.nextTick( callback );
				self.logMonitor(9, "Calling docker...");
				perf.begin('docker');
				cp.execFile( self.dockerBin, ['stats', '--no-stream', '--format', 'json'], { timeout: 10000 }, function(err, stdout, stderr) {
					perf.end('docker');
					if (err) self.logMonitor(9, "Docker stats error: " + err);
					else if (stdout.length) {
						self.logMonitor(9, "Docker stats response", { stdout, stderr });
						var docker = info.data.docker = { containers: [], totals: { cpu: 0, mem: 0, net: 0, disk: 0, pids: 0 } };
						stdout.trim().split(/\n/).forEach( function(line) {
							if (!line.match(/\S/)) return;
							try { docker.containers.push( self.parseDockerStatRow(JSON.parse(line)) ); } catch (e) {;}
						} );
						docker.containers.forEach( function(cont) {
							docker.totals.cpu += cont.cpu || 0;
							docker.totals.mem += cont.mem_usage || 0;
							docker.totals.net += cont.net_total || 0;
							docker.totals.disk += cont.disk_total || 0;
							docker.totals.pids += cont.pids || 0;
						} );
					}
					callback();
				}); // cp.execFile
			...
			function(callback) {
				// all done
				perf.end();
				var metrics = info.data.metrics = perf.metrics();
		...
		function() {
			self.monitorInProgress = false;
			self.monitorOverrunLogged = false;
	...
	runMonitorCommand(command, callback) {
		...
		child.on('exit', function (code, signal) {
			...
			if ((command.format == 'json') && result.match(/(\{|\[)/)) {
				...
				catch (err) { result = 'Error: JSON Parser Error: ' + err; }
				if (json) result = json;
			}
				...
				catch (err) { result = "Error: XML Parser Error: " + err; }
				if (xml) result = xml;
			}
			else {
